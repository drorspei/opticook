...
import { ChefHat, Play, Loader } from 'lucide-react';
import { api } from '../api';

export const SessionSetup = ({ onSessionStarted }) => {
  const [recipes, setRecipes] = useState([]);
  const [selectedRecipe, setSelectedRecipe] = useState('');
  const [chefNames, setChefNames] = useState(['Alice', 'Bob']);
  ...
  const updateChefName = (index, name) => {
